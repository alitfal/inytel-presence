...
<template>
  <div class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4"
    ...
    <div class="bg-white rounded-3xl w-full max-w-4xl shadow-2xl max-h-[90vh] overflow-y-auto">
      <!-- Cabecera -->
      <div class="flex items-center gap-4 px-8 pt-8 pb-6 border-b border-slate-100">
        <div
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-0 md:divide-x md:divide-slate-100">
        ...
        <div class="px-8 py-6 space-y-6">
          ...
          <div class="text-sm">
            <div class="flex justify-between py-3 border-b border-slate-100">
              ...
              <span :class="employee.estado === 'DENTRO'
                  ? 'text-emerald-500'
                  : 'text-slate-400'
            ...
            <div class="flex justify-between py-3">
              ...
              <span class="text-slate-700 font-medium">{{ employee.horas_semanales || 40 }} h/semana</span>
            </div>
          </div>
          ...
          <div>
            <div class="flex items-center justify-between mb-3">
              ...
              <div class="flex items-center gap-1">
                ...
                <button @click="cambiarSemana(1)" :disabled="semanaOffset >= 0"
            ...
            <div v-if="loadingHoras" class="text-center py-4 text-indigo-600 text-sm font-bold animate-pulse">
              Cargando horas...
            </div>
            ...
            <template v-else-if="resumenHoras">
              <div class="grid grid-cols-3 gap-3 mb-4">
                ...
                <div class="rounded-2xl p-3 text-center" :class="resumenHoras.diferencia_total >= 0
                    ? 'bg-emerald-50'
                    : resumenHoras.diferencia_total >= -2
                      ? 'bg-amber-50'
                      : 'bg-rose-50'
                  ">
                  <p class="text-[10px] font-bold uppercase tracking-wider mb-1" :class="resumenHoras.diferencia_total >= 0
                      ? 'text-emerald-400'
                      : resumenHoras.diferencia_total >= -2
                        ? 'text-amber-400'
                        : 'text-rose-400'
              ...
              <div class="space-y-2">
                <div v-for="dia in resumenHoras.dias" :key="dia.fecha"
                  class="flex items-center gap-3 bg-slate-50 rounded-xl px-3 py-2">
                  <span class="text-xs font-bold text-slate-400 w-10 capitalize">{{ formatDia(dia.fecha) }}</span>
                  <div class="flex-1 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                    <div class="h-full rounded-full transition-all" :class="dia.diferencia >= 0 ? 'bg-emerald-500' : 'bg-rose-400'
                      " :style="{
                        width:
                          Math.min(
                  ...
                  <span class="text-xs font-medium shrink-0 text-right w-14" :class="colorDiferencia(dia.diferencia)">
        ...
        <div class="px-8 py-6">
          ...
          <div class="flex gap-2 mb-3">
            <button
              v-for="p in [{ key: 'hoy', label: 'Hoy' }, { key: 'semana', label: 'Semana' }, { key: 'mes', label: 'Mes' }]"
              :key="p.key" @click="cambiarPeriodo(employee.id, p.key)"
              :class="periodoActivo === p.key ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'"
          ...
          <div class="flex items-center justify-between mb-4">
            <button @click="navegarPeriodo(employee.id, -1)"
              class="p-1 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors cursor-pointer">
              <ChevronLeft class="w-4 h-4" />
            </button>
            <span class="text-xs font-medium text-slate-500 text-center">{{ etiquetaPeriodo }}</span>
            <button @click="navegarPeriodo(employee.id, 1)" :disabled="offset >= 0"
              class="p-1 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors cursor-pointer disabled:opacity-30">
              <ChevronRight class="w-4 h-4" />
            </button>
          </div>
          ...
          <div v-else-if="historial.length === 0" class="text-center py-4 text-slate-400 text-sm">
            Sin fichajes en este periodo
          </div>
          <div v-else class="space-y-2 overflow-y-auto pr-1" style="max-height: 560px">
            <div v-for="j in historial" :key="j.id" class="bg-slate-50 rounded-2xl px-4 py-3"
              ...
              <div class="flex items-center justify-between mb-1">
                <span class="text-xs font-bold text-slate-500 capitalize">{{ formatFecha(j.fecha_entrada) }}</span>
              ...
              <div class="flex items-center gap-2 text-xs">
                ...
                <span :class="j.hora_salida ? 'bg-rose-100 text-rose-500' : 'bg-slate-100 text-slate-400'"
              ...
              <div v-if="j.motivo_incidencia && j._expandido" class="mt-3 pt-3 border-t border-amber-100 space-y-2">
                <div class="flex justify-between text-xs">
                  ...
                  <span class="font-bold text-amber-600 capitalize">
                    {{ j.motivo_incidencia === "olvido" ? "Olvido de fichaje" : j.motivo_incidencia ===
                      "error_aplicacion" ?
                    "Error de aplicación" : "Otros" }}
                  </span>
                </div>
                <div v-if="j.hora_salida_real" class="flex justify-between text-xs">
                  ...
                  <span class="font-bold text-slate-700">{{ j.hora_salida_real?.slice(0, 5) }}</span>
                </div>
