<script setup>
/**
 * EmployeeForm.vue — Formulario de creación y edición de empleados
 *
 * Modal de formulario que funciona en dos modos:
 * - Creación: campos vacíos, botón "Crear empleado"
 * - Edición: campos precargados, botón "Guardar cambios"
 *
 * El componente es puramente presentacional — no gestiona
 * la lógica de validación ni el estado del formulario.
 * Toda la lógica reside en el composable useEmployeeForm.js
 * y se comunica mediante props y eventos.
 *
 * Incluye un selector visual de días laborables y un campo
 * de horas semanales que muestra automáticamente las horas
 * diarias equivalentes según los días seleccionados.
 *
 * Props:
 * @prop {Object}   formData      - Datos reactivos del formulario
 * @prop {Object}   erroresCampo  - Errores de validación por campo
 * @prop {Object}   tocados       - Campos que el usuario ha interactuado
 * @prop {Boolean}  modoEdicion   - true = editar, false = crear
 * @prop {Function} claseCampo    - Devuelve la clase CSS del borde según estado
 * @prop {Function} marcarTocado  - Marca un campo como tocado al perder el foco
 *
 * Eventos emitidos:
 * @emits guardar   - Solicita guardar el formulario (validación en el padre)
 * @emits cancelar  - Solicita cerrar el modal sin guardar
 */
...
// Definición de los días de la semana para el selector visual
// num = valor numérico (1=lunes, 7=domingo), label = inicial mostrada
const diasSemana = [
  { num: 1, label: "L" },
  { num: 2, label: "M" },
  { num: 3, label: "X" },
...
/**
 * Computed bidireccional que convierte el string de días laborables
 * almacenado en formData ("1,2,3,4,5") a un array de números ([1,2,3,4,5])
 * para facilitar la manipulación en el selector visual.
 *
 * get: string → array
 * set: array → string ordenado
 */
...
/**
 * Activa o desactiva un día en el selector.
 * Impide dejar cero días seleccionados (mínimo 1 día laborable).
 *
 * @param {number} num - Número del día a alternar (1-7)
 */
function toggleDia(num) {
  ...
  if (idx === -1) {
    ...
    // Solo elimina si quedan más de un día seleccionado
    actual.splice(idx, 1);
  }
  ...
  >
    ...
    >
      ...
      <div class="space-y-4 text-sm">
        ...
        <!-- DNI — usado también como contraseña temporal al crear el empleado -->
        <div>
        ...
        <!--
          Horas semanales — muestra automáticamente las horas diarias
          equivalentes según el número de días laborables seleccionados
        -->
        <div>
          <label class="text-slate-400 font-medium block mb-1"
        ...
        <!--
          Selector de días laborables — botones toggle visuales
          Mínimo 1 día seleccionado, ordenados de lunes a domingo
        -->
        <div>
