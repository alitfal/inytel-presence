...
// ── Horas semanales ─────────────────────────────────────────────
// Reemplaza el bloque de computed horasSemanales y la constante HORAS_OBJETIVO por esto:
...
const HORAS_MES = computed(() => {
  ...
  const semanasAprox = diasEnMes / 7;
  return Math.round(HORAS_SEMANA.value * semanasAprox);
});
  ...
  >
    ...
    <main class="max-w-lg mx-auto px-6 py-10">
      ...
      <template v-else-if="empleado">
        <!-- Tarjeta perfil -->
        <div
          class="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm mb-6"
        >
          ...
          <div class="text-sm space-y-0">
            <div class="flex justify-between py-3 border-b border-slate-100">
              ...
              <span class="text-slate-700 font-medium">{{ empleado.dni }}</span>
            </div>
            <div class="flex justify-between py-3 border-b border-slate-100">
